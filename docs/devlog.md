...
## V7.1c fixes de la re-auditoría (2026-07-11)

Cerró las brechas 9-13 de la re-auditoría: global del paquete **62 %→68 %** (re-calificado en
`docs/benchmark/faja-paqueteria-4m/2026-07-11/`, misma rúbrica-v1, base honesta = el 62 %
re-auditado). Cuatro frentes, todos verificados en el paquete regenerado (25/25 artefactos):

**A · La memoria lee del MODELO, no defaults** (`library/rules.py` + `engineering/report.py`).
El bug de raíz: filtros por SUBSTRING contaban piezas que solo MENCIONAN un rol. Fix por ROL
(nombre —o su parte tras « · »— empieza por el rol): `_LEG_RE`/`_LARG_RE`/`_EJE_RE`. (A1) Pandeo:
6 patas, no 8 («Ménsula … → larguero + pata» y «Disco … a la pata» ya no cuentan; además solo
columnas ≥50 mm). (A2) Flecha: 2 largueros, no 4 (las ménsulas que mencionan «larguero» fuera).
(A3) Ø del eje motriz = 35 (nombre «Ø35»/geometría del cilindro por rol «Eje»), no la variable
stale `diam_eje=30` ni —bug intermedio cazado en vivo— el «Tambor motriz (… eje vivo)» Ø114 que
el substring «eje»+«motriz» capturaba. FS de flexión 1.43→2.27. (A4) L10 con la carga radial de
...
`structure_engineering_check(belt_radial_n=…)` en `_bearing_checks`. L10 761M→10.3M h, honesto.

**B · Cirugía del modelo 38** (API viva). Patrón de anclaje: 5 pernos faltaban porque `c149`
(`pattern_group` 3×2) replicaba `c148` (3 pernos) pero NO el perno semilla `c147` de cada placa.
Fix paramétrico: `c1114` = `pattern_group` source `c147` con el MISMO 3×2 (`spacing.x=(long_centros
-600)/2`, `spacing2.y=-2*larg_cy`) → los 5 pernos que faltaban, exactamente en el 4º barreno de
cada placa, siguiendo `largo_total` en ambas variantes. 19→24 (get_scene/BOM=24). `c704`
(«Tornillería ménsula soporte motor», flotante/sin grupo): `fasten` contacto→c703 + grupo
Transmision; los 5 pernos nuevos grounded (embebidos en piso) + contacto a su placa → soundness
**0 flotantes** (era 1), gravity 0 caídas, DOF 214. Revisión 80.

**C · Compras vs fabricación + títulos** (`drawing/sheetset.py` + `library/cutlist.py`). Las
piezas de COMPRA modeladas a-medida (pernos/pies/banda, detectadas por ROL en `_PURCHASE_RE`
tolerando el prefijo de sub-ensamblaje) salen de `cut_list` (ni lámina de fabricación ni fila de
corte) → van a `hardware_schedule`, que ahora cuenta EXACTO como el BOM (nombre base sin sufijo de
instancia, +1 por feature) — 24 pernos, no 24 filas ni 16 «pies» por sólido. Juego 32→22 pág.
`sheet_set` pasa el título real a las hojas de tabla (ya no «Sin título») y rotula la SECCIÓN de
catálogo («HSS 76.2×76.2×3») en la lámina de una pieza de perfil. C4: la cédula = el BOM en
cantidades; residuo declarado = «Ménsulas de chumacera» 2 (cutlist, brácketes físicos) vs 1 (BOM,
ítem de conjunto), convención con TOTAL idéntico.

**D · `benchmark_package.py` endurecido**: exit≠0 ante fallos (regresión de CI real), gate de
estado (`--expect largo_total=4000`, health.ok, suppressed) que aborta ANTES de generar sobre un
doc degradado, `--out` derivado del slug del proyecto, `--checks` para calificar proyectos ≠38
(las aserciones `verify` eran 38-específicas; sin ellas se OMITE, no se inventa), `validacion.json`
